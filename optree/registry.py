...
def _pytree_node_registry_get(
    ...
    *,
    ...
    if not _C.is_dict_sorted(namespace):
        if cls is dict:
...
def dict_sorted(mode: bool, *, namespace: str) -> Generator[None, None, None]:
    ...
    >>> with dict_sorted(False, namespace='some-namespace'):  # doctest: +IGNORE_WHITESPACE
    ...
    with __REGISTRY_LOCK:
        prev = _C.is_dict_sorted(namespace)
        _C.set_dict_sorted(mode, namespace)

    try:
        yield
    finally:
        with __REGISTRY_LOCK:
            _C.set_dict_sorted(prev, namespace)
