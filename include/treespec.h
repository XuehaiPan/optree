...
class PyTreeSpec {
 ...
 public:
    ...
    // Check if should not preserve the insertion order of the dictionary keys during flattening.
    inline static bool IsDictSorted(const std::string &registry_namespace) {
        return sm_is_dict_insertion_ordered.find(registry_namespace) ==
               sm_is_dict_insertion_ordered.end();
    }

    // Set the namespace to not preserve the insertion order of the dictionary keys during
    // flattening.
    inline static void SetDictSorted(const bool &mode, const std::string &registry_namespace) {
        if (mode) [[unlikely]] {
 ...
 public:
    PyTreeIter(const py::object &tree,
        ...
        : m_agenda({std::make_pair(tree, 0)}),
          ...
          m_is_dict_insertion_ordered(!PyTreeSpec::IsDictSorted(registry_namespace)) {};
